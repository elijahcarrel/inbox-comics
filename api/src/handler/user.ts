...
import { createUser as serviceCreateUser } from "../service/user";
import { putUser as servicePutUser } from "../service/user";
...
export const putUser = async (_: any, args: { publicId: string; user: InputUser }) =>
    servicePutUser(args.publicId, args.user)

export const resendVerificationEmail = async (_: any, args: { email: string }) => {
    const { email } = args;
    const user = await User.findOne({ email }).exec();
    if (user == null) {
        throw invalidUserError(email);
